...
const COMMERCIAL_ANGLES = ["av_droit", "ar_gauche"]; // Photos à détourage automatique
...
async function composeCommercialPhoto(subjectBase64, immat, logoB64) {
  return new Promise(res => {
    ...
    const barH = 130; // bande du bas plus grande

    // Fond gris très clair
    ctx.fillStyle = "#f2f3f5";
    ctx.fillRect(0, 0, W, H);

    // Bande bleue en bas (plus grande)
    ...
    subject.onload = () => {
      // Zoom léger : 90% de la largeur, 88% de la hauteur dispo
      const maxW = W * 0.90;
      const maxH = (H - barH - 5) * 0.88;
      ...
      logo.onload = () => {
        const logoH = 68, logoW = (logo.naturalWidth / logo.naturalHeight) * logoH;
        const tmpC = document.createElement("canvas");
        tmpC.width = logoW; tmpC.height = logoH;
        const tmpCtx = tmpC.getContext("2d");
        tmpCtx.filter = "brightness(0) invert(1)";
        tmpCtx.drawImage(logo, 0, 0, logoW, logoH);
        ctx.drawImage(tmpC, 48, H - barH + (barH - logoH)/2, logoW, logoH);

        // Séparateur
        ctx.strokeStyle = "rgba(255,255,255,0.25)";
        ctx.lineWidth = 1;
        ctx.beginPath(); ctx.moveTo(logoW+72, H-barH+20); ctx.lineTo(logoW+72, H-20); ctx.stroke();

        if (immat) {
          ctx.fillStyle = "#ffffff";
          ctx.font = "700 36px monospace";
          ctx.textAlign = "right";
          ctx.fillText(immat, W - 48, H - barH + barH/2 + 13);
        }
            ...
            {depStep===2&&(
              <div>
                ...
                <div className="card" style={{marginBottom:10}}>
                  <div className="g3">
                    {[["Immatriculation",depForm.immat],["Type nacelle",depForm.type_nacelle],["Modèle",depForm.modele],["Année fabrication",depForm.annee_fab],["Client",depForm.client],["Contrat",depForm.contrat],["Email",depForm.email],["Date",depForm.date],["Heures nacelle",depForm.heures?depForm.heures+" h":"—"],["Km porteur",depForm.km_porteur?depForm.km_porteur+" km":"—"],["Agent",depForm.agent]].map(([k,v])=>(
                ...
                {foundDossier&&(
                  <div>
                    <div className="card" style={{marginBottom:14,border:"2px solid var(--primary)"}}>
                      ...
                      <div className="g3">
                        {[["Immatriculation",foundDossier.immat],["Type nacelle",foundDossier.info?.type_nacelle],["Modèle",foundDossier.info?.modele],["Client",foundDossier.info?.client],["Contrat",foundDossier.info?.contrat],["Départ",foundDossier.depart?.date],["Année fabrication",foundDossier.info?.annee_fab],["Heures départ",foundDossier.depart?.heures?foundDossier.depart.heures+" h":"—"],["Km porteur départ",foundDossier.depart?.km_porteur?foundDossier.depart.km_porteur+" km":"—"]].map(([k,v])=>(
