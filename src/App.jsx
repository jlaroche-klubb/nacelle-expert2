...
:root{
  ...
  --primary:#1a2a6e;--accent:#c8102e;--gold:#1a2a6e;--gold2:#2a3a8e;
...
.badge-danger{background:rgba(200,16,46,.1);color:var(--accent);border:1px solid rgba(200,16,46,.3);}
...
.modal{background:#fff;border:1px solid var(--border2);padding:28px;min-width:340px;max-width:560px;width:90%;max-height:85vh;overflow-y:auto;box-shadow:0 8px 32px rgba(0,0,0,.15);}
...
@media print{
  ...
  .header-bar{background:var(--primary)!important;-webkit-print-color-adjust:exact;}
  .total-strip{background:var(--primary)!important;-webkit-print-color-adjust:exact;}
}
`;
...
function pickFile(opts={}) {
  return new Promise(res => {
    const inp=document.createElement("input"); inp.type="file"; inp.accept=opts.accept||"image/*"; inp.multiple=opts.multiple||false; inp.onchange=()=>res(inp.files); inp.click();
  });
}

async function compressBase64(base64,maxW=800,quality=0.7) {
  return new Promise(res => {
    const img=new Image(); img.onload=()=>{ const canvas=document.createElement("canvas"); const ratio=Math.min(maxW/img.width,maxW/img.height,1); canvas.width=img.width*ratio; canvas.height=img.height*ratio; canvas.getContext("2d").drawImage(img,0,0,canvas.width,canvas.height); res(canvas.toDataURL("image/jpeg",quality)); }; img.src=base64;
  });
}
...
async function fbSaveDossier(data) {
  try {
    ...
    if(compressed.depart?.photos) compressed.depart={...compressed.depart,photos:await compressPhotos(compressed.depart.photos)};
    if(compressed.retour?.photos) compressed.retour={...compressed.retour,photos:await compressPhotos(compressed.retour.photos)};
...
export default function App() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      setDossiers(result);
      const zConf=await fbGetConfig("zones"); if(zConf?.data) setZones(zConf.data);
      const tConf=await fbGetConfig("tarifs"); if(tConf?.data) setTarifs(tConf.data);
  ...
  async function saveZone() {
    ...
    setZones(updated); await fbSaveConfig("zones",{data:updated});
    setZoneForm({label:"",icon:"⟋"}); setZoneEdit(null); flash(zoneEdit!==null?"Zone modifiée ✓":"Zone ajoutée ✓");
  }

  async function deleteZone(idx) {
    const z=zones[idx]; const uz=zones.filter((_,i)=>i!==idx); const ut=tarifs.filter(t=>t.zone!==z.id);
    setZones(uz); setTarifs(ut); await fbSaveConfig("zones",{data:uz}); await fbSaveConfig("tarifs",{data:ut}); flash("Zone supprimée ✓");
  }

  async function saveTarif() {
    if(!tarifForm.label.trim()||!tarifForm.zone||!tarifForm.prix) return;
    let updated;
    if(tarifEdit!==null) updated=tarifs.map((t,i)=>i===tarifEdit?{...t,label:tarifForm.label,zone:tarifForm.zone,prix:parseInt(tarifForm.prix)}:t);
    else { const id=tarifForm.label.toLowerCase().replace(/[^a-z0-9]/g,"_").slice(0,18)+"_"+genId().slice(0,3).toLowerCase(); updated=[...tarifs,{id,zone:tarifForm.zone,label:tarifForm.label,prix:parseInt(tarifForm.prix)}]; }
    setTarifs(updated); await fbSaveConfig("tarifs",{data:updated});
    setTarifForm({zone:"",label:"",prix:""}); setTarifEdit(null); flash(tarifEdit!==null?"Poste modifié ✓":"Poste ajouté ✓");
  }
      ...
      <div className="header-bar no-print">
        <div style={{display:"flex",alignItems:"center",gap:16,cursor:"pointer"}} onClick={goHome}>
          ...
          <div>
            ...
            <div style={{fontSize:10,letterSpacing:2,color:"rgba(255,255,255,.6)",textTransform:"uppercase"}}>Système d'expertise PEMP</div>
          </div>
        </div>
            ...
            {depStep===0&&(
              <div>
                ...
                <div className="card" style={{marginBottom:14}}>
                  ...
                  <div style={{marginTop:12}}><label>Agent expert</label><input value={depForm.agent} onChange={e=>setDepForm({...depForm,agent:e.target.value})} placeholder="Prénom Nom"/></div>
                </div>
                  ...
                  <div key={zone.id} className="zone-row">
                    <div className="zone-header" onClick={()=>setOpenZone(openZone===zone.id?null:zone.id)}>
                      ...
                      <div style={{display:"flex",gap:8,alignItems:"center"}}>
                        ...
                        {zone.id==="tour_complet"&&TOUR_ANGLES.filter(a=>depPhotos[`tour_complet_${a.key}`]?.[0]).length>0&&<span className="badge badge-ok">{TOUR_ANGLES.filter(a=>depPhotos[`tour_complet_${a.key}`]?.[0]).length}/4 photos</span>}
                    ...
                    {openZone===zone.id&&(
                      <div className="zone-body">
                        {zone.id==="tour_complet"?(
                          <div>
                            <div style={{fontSize:11,color:"var(--muted)",marginBottom:14,lineHeight:1.5}}>Photographiez la nacelle sous les 4 angles standards.</div>
                            {TOUR_ANGLES.map((angle,idx)=>{
                              ...
                              return (
                                <div key={angle.key} style={{marginBottom:12,padding:"12px 14px",border:"1px solid var(--border)",background:"#f8f9fb"}}>
                                  ...
                                  <div style={{display:"flex",gap:8,alignItems:"center"}}>
                                    {photo?(<div style={{position:"relative"}}><img src={photo.url} alt="" style={{width:120,height:88,objectFit:"cover",border:"1px solid var(--border2)"}}/><button className="btn btn-danger" onClick={()=>removePhoto(key,0,setDepPhotos)} style={{position:"absolute",top:2,right:2,padding:"2px 5px",fontSize:10}}>✕</button></div>):(<div className="photo-add" style={{width:120,height:88}} onClick={async()=>{const f=await pickFile({multiple:false});if(f) addPhotos(f,key,setDepPhotos);}}>+</div>)}
                                    <div style={{fontSize:11,color:"var(--muted)",lineHeight:1.6}}>
                                      {angle.key==="av_droit"&&"Avant droit, environ 5m, machine entière."}
                                      {angle.key==="av_gauche"&&"Avant gauche, environ 5m, machine entière."}
                                      {angle.key==="ar_gauche"&&"Arrière gauche, environ 5m, machine entière."}
                                      {angle.key==="ar_droit"&&"Arrière droit, environ 5m, machine entière."}
                                    </div>
                                  </div>
                                </div>
                              );
            ...
            {depStep===2&&(
              <div>
                ...
                <div className="card" style={{marginBottom:10}}>
                  <div className="g3">
                    {[["Immatriculation",depForm.immat],["Type nacelle",depForm.type_nacelle],["Modèle",depForm.modele],["Client",depForm.client],["Contrat",depForm.contrat],["Email",depForm.email],["Date",depForm.date],["Heures nacelle",depForm.heures?depForm.heures+" h":"—"],["Km porteur",depForm.km_porteur?depForm.km_porteur+" km":"—"],["Agent",depForm.agent]].map(([k,v])=>(
          ...
          <div className="fade-in">
                ...
                {foundDossier&&(
                  <div>
                    <div className="card" style={{marginBottom:14,border:"2px solid var(--primary)"}}>
                      ...
                      <div className="g3">
                        {[["Immatriculation",foundDossier.immat],["Type nacelle",foundDossier.info?.type_nacelle],["Modèle",foundDossier.info?.modele],["Client",foundDossier.info?.client],["Contrat",foundDossier.info?.contrat],["Départ",foundDossier.depart?.date],["Heures départ",foundDossier.depart?.heures?foundDossier.depart.heures+" h":"—"],["Km porteur départ",foundDossier.depart?.km_porteur?foundDossier.depart.km_porteur+" km":"—"]].map(([k,v])=>(
            ...
            {retStep===1&&foundDossier&&(
              <div>
                        ...
                        <div className="zone-body">
                          {zone.id==="tour_complet"?(
                            ...
                            <div>
                              ...
                              {/* DÉGÂTS INLINE */}
                              {tarifs.filter(t=>t.zone===zone.id).length>0&&(
                                <div>
                                  <div style={{fontSize:10,letterSpacing:2,color:"var(--primary)",textTransform:"uppercase",marginBottom:6,fontWeight:700}}>Dégâts constatés</div>
                                  {tarifs.filter(t=>t.zone===zone.id).map(t=>(
                ...
                {retDegats.length>0&&(
                  <div className="total-strip" style={{marginTop:14,marginBottom:8}}>
                ...
                <div className="card" style={{marginBottom:14,border:"2px solid var(--primary)"}}>
                  ...
                  {retDegats.length>0&&(
                    <div>
                      ...
                      {retDegats.map(id=>{const t=tarifs.find(t=>t.id===id);return t?(<div key={id} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:"1px solid var(--border)",fontSize:13}}><span>{t.label}</span><span className="mono" style={{color:"var(--primary)",fontWeight:700}}>{t.prix} €</span></div>):null;})}
                  ...
                  {retDegats.length===0&&<div style={{padding:"12px 0",color:"#208040",fontWeight:600}}>✓ Aucun dégât — nacelle rendue conforme</div>}
                </div>
                <div className="card" style={{marginBottom:14}}>
          ...
          <div className="fade-in">
            ...
            {activeDossier.retour?.degats?.length>0&&(
              <div style={{marginTop:14}}>
                <div style={{fontSize:10,letterSpacing:3,color:"var(--primary)",textTransform:"uppercase",marginBottom:8,fontWeight:700}}>Chiffrage retenue</div>
                <div className="card" style={{marginBottom:10}}>
                  {activeDossier.retour.degats.map(id=>{const t=tarifs.find(t=>t.id===id);return t?(<div key={id} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid var(--border)",fontSize:13}}><span>{t.label} <span style={{fontSize:11,color:"var(--muted)"}}>· {zones.find(z=>z.id===t.zone)?.label}</span></span><span className="mono" style={{color:"var(--primary)",fontWeight:700}}>{t.prix} €</span></div>):null;})}
            ...
            {!adminAuthed?(
              ...
              <div>
                ...
                <div style={{display:"flex",borderBottom:"2px solid var(--primary)",marginBottom:18}}>
                  {[["zones","Zones"],["tarifs","Postes tarifaires"]].map(([id,label])=>(<div key={id} className={`tab ${adminTab===id?"active":""}`} onClick={()=>{setAdminTab(id);setZoneEdit(null);setTarifEdit(null);}}>{label}</div>))}
                </div>
                    ...
                    {zones.map((z,i)=>(
                      <div key={z.id} className="admin-row">
                        <div style={{display:"flex",alignItems:"center",gap:10}}>
                          ...
                          <span style={{fontSize:10,color:"var(--muted)"}}>{tarifs.filter(t=>t.zone===z.id).length} poste{tarifs.filter(t=>t.zone===z.id).length!==1?"s":""}</span>
                        </div>
                    ...
                    {zones.map(zone=>{
                      ...
                      return (
                        <div key={zone.id} style={{marginBottom:12}}>
                          ...
                          {tz.map(t=>{const gi=tarifs.findIndex(x=>x.id===t.id);return(
